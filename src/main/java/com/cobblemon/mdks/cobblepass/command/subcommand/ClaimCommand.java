...
public class ClaimCommand extends Subcommand {
    public ClaimCommand() {
        super("§9Uso: §3/battlepass claim <nivel> [premium]");
    }
    ...
    private int run(CommandContext<CommandSourceStack> context, boolean hasPremiumArg) {
        ...
        if (tier == null) {
            player.sendSystemMessage(Component.literal(Constants.ERROR_PREFIX + "¡Nivel inválido!"));
            return 1;
        }
        ...
        if (premium) {
            ...
            if (level > pass.getLevel()) {
                player.sendSystemMessage(Component.literal(Constants.ERROR_PREFIX + "¡Aún no has alcanzado el nivel " + level + "!"));
                return 1;
            }
