...
public class AddLevelsCommand extends Subcommand {
    ...
    public AddLevelsCommand() {
        super("§9Uso:\n§3- /battlepass addlevels [jugador] <niveles>");
    }
    ...
    public int run(CommandContext<CommandSourceStack> context) {
        try {
            ...
            if (target == null) {
                context.getSource().sendSystemMessage(Component.literal("§cError: Jugador no encontrado"));
                return 0;
            }
            ...
            if (currentLevel + levelsToAdd > 100) {
                ...
                if (levelsToAdd <= 0) {
                    context.getSource().sendSystemMessage(Component.literal("§cError: El jugador ya está en el nivel máximo"));
                    return 0;
                }
            }
            ...
            String message = String.format("§aAgregados %d niveles al pase de batalla de %s",
            ...
            if (context.getSource().getPlayer() != target) {
                target.sendSystemMessage(Component.literal(
                    String.format("§a¡Recibiste %d niveles del pase de batalla!", levelsToAdd)
                ));
            }
